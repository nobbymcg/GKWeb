...
var mainSection = document.querySelector('.column.main.window');
...
// Retrieve colors from CSS file
// Assumes the CSS variable is defined on the :root pseudo-class (which they are)
var rootStyles = getComputedStyle(document.documentElement);

var gkwebBlue = rootStyles.getPropertyValue('--gkweb_blue').trim();
var gkwebLightBlue = rootStyles.getPropertyValue('--gkweb_lightblue').trim();
var gkwebGrey = rootStyles.getPropertyValue('--gkweb_grey').trim();
var gkwebDarkGrey = rootStyles.getPropertyValue('--gkweb_darkgrey').trim();
var navHighlight = gkwebBlue;
var navNormal = 'white';
var canvasSize = rootStyles.getPropertyValue('--gkweb_canvassize').trim();

var scopeBackgroundColor = gkwebGrey;
var scopeLineColor = 'rgb(128, 255, 128)';
var scopeCircleMargin = 100;

// This is mad but needed to prevent the canvas from being the wrong shape
...
// Store last item highlighted in nav bar
var currentNavItem = document.getElementById('verify_icon');
currentNavItem.style.stroke = navHighlight;
    ...
    var onSuccess = function(stream) {
      ...
      record.onclick = function() {
        ...
        if (recordState == false) {
          ...
          record.style.backgroundColor = gkwebGrey;
...
async function displayAudioAndResults(){
    ...
    if (verifyWithMiddleware(blobToSend, clipResultImage, resultsHolder) == 'ERROR') {
        resultsHolder.textContent = "Failed to connect to Middleware";
    }
}
...
function showGKLogo(){
  ...
  if (recordState == false) {
    canvasCtx.beginPath();
    canvasCtx.lineWidth = 2;
    canvasCtx.strokeStyle = gkwebBlue;
...
function verifyWithMiddleware(blob, imageHolder, resultsHolder){
    ...
    var claimedID = document.getElementById('claimedID').value;
    console.log("Claimed ID is " + claimedID);
    // var requestURL = "http://localhost:8080/verify/IanMcG/TI";
    var requestURL = "http://localhost:8080/verify/"+ claimedID +"/TI";
    console.log("Sending request to " + requestURL);
    if (request.open("POST", requestURL)) {
        request.send(blob);
        var response = '';
        ...
        imageHolder.src = 'images/gatekeeper_grey.svg';
        lastResult = 'DECISION_UNKNOWN';
        console.log("Failed to open connection to Middleware");
        return('ERROR');
    }
}

// function to change the stroke color of the item when clicked
function highlightNavItem(id) {
    // revert previously highlighted item to original color
    currentNavItem.style.stroke = navNormal;
    // change sroke color of new currentNavItem
    currentNavItem = document.getElementById(id);
    currentNavItem.style.stroke = navHighlight;
}
